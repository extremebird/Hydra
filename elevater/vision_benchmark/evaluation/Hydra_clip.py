...
import gc
from ..evaluation.clip_load import Hydra_load
...
def get_cls_model(config, feature_type='image'):
    ...
    model, _ = Hydra_load(config.MODEL.NAME, jit=False)
...
class Classifier(torch.nn.Module):
    ...
    def get_Hydra_weights(self):
        hydra_dict = OrderedDict()

        for name, param in self.backbone.named_parameters():
            if 'adapter' in name:
                hydra_dict[name] = param
...
def train_task(train_dataloader, test_dataloader, config, sweep_run=False, final_run=False):
    best_acc1 = 0

    model = Classifier(config, 0)
    pytorch_total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    logger.info(f'Number of trainable params: {pytorch_total_params / 1000000}M.')
    logger.info(f"=> model detail is {model}.")
    ...
    for epoch in range(config.TRAIN.BEGIN_EPOCH, config.TRAIN.END_EPOCH):
        ...
        if acc1 > best_acc1:
            ...
            model_info['Hydra_weights'] = model.get_Hydra_weights()
...
def train_one(train_loader, model, criterion, optimizer, epoch, config):
    ...
    try:
        ...
        metric_result = 0.
    logger.info(f'[Epoch {epoch}] Train: {metric_name} {metric_result:.3f}')
...
def ours_tuning_clip(train_dataloader, val_dataloader, test_dataloader, no_hyperparameter_tuning, lr, l2, config):
    ...
    if config.DATASET.DATASET == 'patch-camelyon' and config.DATASET.NUM_SAMPLES_PER_CLASS == 10000:
        ...
        logger.info(f'Generated: len(train)={len(train_dataloader.dataset)}, len(val)={len(val_dataloader.dataset)}, len(test)={len(test_dataloader.dataset)}.')
    ...
    return train_task(trainval_dataloader, test_dataloader, config, final_run=True)

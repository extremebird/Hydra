...
class ModelArguments:
    ...
    apply_hydra: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to apply hydra or not"}
    )
    hydra_par_r: Optional[int] = field(
        default=None,
        metadata={"help": "hydra parallel r"}
    )
    hydra_seq_r: Optional[int] = field(
        default=None,
        metadata={"help": "hydra sequential r"}
    )
    hydra_path: Optional[str] = field(
        default=None,
        metadata={"help": "The file path of hydra parameters."}
    )
    hidden_dropout_prob: Optional[float] = field(
        default=None,
        metadata={"help": "LoLA FFN dropout probability"}
    )
...
def main():
    ...
    config = AutoConfig.from_pretrained(
        ...
        apply_hydra=model_args.apply_hydra,
        hydra_par_r=model_args.hydra_par_r,
        hydra_seq_r=model_args.hydra_seq_r,
    ...
    if model_args.apply_hydra:
        trainable_params.append('hydra')
    
    if model_args.hydra_path is not None:
        hydra_state_dict = torch.load(model_args.hydra_path)
        model.load_state_dict(hydra_state_dict, strict=False)

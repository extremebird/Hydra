...
def get_dataloader(dataset, val_split=0.0, batch_size_per_gpu=64, workers=6, pin_memory=True):
    if val_split == 0:
        ...
        def train_val_dataset(dataset, val_split):
            ...
            logger.info('Quick fetch label starts.')
            labels = quick_fetch_labels(dataset)
            logger.info('Quick fetch label finished.')
            # logger.info('Full fetch label starts.')
            # labels_all_fetch = np.asarray([x[1] for x in dataset])
            # logger.info('Full fetch label finished.')
            # assert (labels == labels_all_fetch).all()
            # logger.info('Quick fetch label same as full fetch.')
...
def load_custom_ic_model(config):
    logger.info(f'=> Loading custom model {config.MODEL.NAME}.')
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    torch.device("cuda")
    logger.info(f'Load model in device {device}.')

    model = eval(config.MODEL.NAME + '.get_cls_model')(config)
    model_file = config.TEST.MODEL_FILE
    logger.info(f'=> load model file: {model_file}')
    ext = model_file.split('.')[-1]
    if ext == 'pth':
        state_dict = torch.load(model_file, map_location="cpu")
    elif ext == 'pkl':
        logger.info('=> load pkl model')
...
def get_model(config, feature_type='image'):
    ...
    if model_name in dir(torchvision.models):
        ...
        logger.info(f'Using Pytorch pretrained model {model_name}')
        ...
        logger.info(f'Using Timm pretrained model {model_name}')
        ...
        if config.LOSS.LOSS == 'softmax':
            ...
            logger.info(f'Training objective: { config.LOSS.LOSS }.')
        ...
        if model_name in clip.available_models():
            ...
            if feature_type == 'image':
                ...
                raise Exception('Incorrect model type.')
            logger.info(f'Using CLIP pretrained model {model_name}, input size {model.visual.input_resolution}')
        else:
            raise ValueError(f'Unknown model name {model_name}.')

    pytorch_total_params = sum(p.numel() for p in model.parameters())
    logger.info(f'Number of params: {pytorch_total_params / 1000000}M.')
    return model
...
def extract_feature(model, data_loader, config):
    ...
    logger.info(f'=> Feature extraction duration time: {time.time() - start:.2f}s')
...
def extract_text_features(config, tokenizer, args=None, model=None, return_numpy=True):
    ...
    logger.info(f'=> Feature extraction duration time: {time.time() - start:.2f}s')
    logger.info(f'=> Knowledge source count | knowledge_count: {wiki_count} | gpt3_count {gpt3_count} ')

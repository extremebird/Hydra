...
class Hydra(nn.Module):
    def __init__(self, in_dim, out_dim, bottleneck_dim, scale="1.0", scale_init_value=1.0, init_option="lora"):
        super().__init__()
        self.down_proj = nn.Linear(in_dim, bottleneck_dim)
        self.up_proj = nn.Linear(bottleneck_dim, out_dim)
        if scale == "learnable":
            self.scale = nn.Parameter(torch.tensor([scale_init_value]), requires_grad=True)
        else:
...
class HydraMLP(nn.Module):
    def __init__(self, in_channel, out_channel):
        super().__init__()
        # First linear layer of the MLP block
        self.linear = nn.Linear(in_channel, out_channel)
        
        # Parallel adapter
        self.adapter_par = Hydra(in_channel, out_channel, 
                                bottleneck_dim=2, scale="1.0",
                                init_option="lora")

        # Sequential adapter        
        self.adapter_seq = Hydra(out_channel, out_channel, 
                                bottleneck_dim=2, scale="1.0",
                                init_option="lora")
    ...
    def forward(self, x):
        ...
        mlp_branch = x
        parallel_branch = self.adapter_par(prev_x)
        sequential_branch = self.adapter_seq(x)

        return mlp_branch + parallel_branch + sequential_branch
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, kattention = None):
        ...
        self.mlp = nn.Sequential(OrderedDict([
            ...
            ("gelu", QuickGELU()),
            ("ad_lin", HydraMLP(d_model * 4, d_model))
        ]))
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.transformer = Transformer(width, layers, heads, kattention = None)
...
class CLIP(nn.Module):
    ...
    def initialize_parameters(self):
        ...
        for block in self.transformer.resblocks:
            ...
            nn.init.normal_(block.mlp.ad_lin.linear.weight, std=proj_std)
...
def build_Hydra_model(state_dict: dict):
    ...
    for k, v in state_dict.items():
        if 'c_proj' in k:
            new_k = k.replace('c_proj', 'ad_lin.linear')

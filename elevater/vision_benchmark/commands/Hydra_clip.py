...
from vision_benchmark.evaluation.Hydra_clip import *
...
def add_finetuning_args(parser):
    ...
    parser.add_argument('--save-model', help='path to save trained model weights.', default=None, type=str)
...
def main():
    ...
    logger.info(f'trainloader: {len(train_dataloader.dataset) + len(val_dataloader.dataset)} ||| testloader: {len(test_dataloader.dataset)}')
    ...
    best_acc, model_info = ours_tuning_clip(train_dataloader, val_dataloader, test_dataloader, args.no_tuning, args.lr, args.l2, config)

...
from transformers.models.adapter import Adapter, Hydra
...
class RobertaOutput(nn.Module):
    def __init__(self, config):
        ...
        self.apply_hydra = False
        if config.apply_hydra:
            self.apply_hyra = True
            self.hydra_par = Hydra(config.intermediate_size, config.hidden_size,
                                    r=config.hyrda_par_r)
            self.hydra_seq = Hydra(config.hidden_size, config.hidden_size,
                                    r=config.hydra_seq_r)

    def forward(self, hidden_states, input_tensor):
        if self.apply_hydra:
            hidden_states1 = self.dense(hidden_states)
            hidden_states2 = self.hydra_par(hidden_states)
            hidden_states3 = self.hydra_seq(hidden_states1)
            hidden_states = hidden_states1 + hidden_states2 + hidden_states3
            hidden_states = self.dropout(hidden_states)
            hidden_states = self.LayerNorm(hidden_states + input_tensor)
            return hidden_states
        else:
